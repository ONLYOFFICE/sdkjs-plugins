...
window.scope = {};
window.apply_scope = function(s)
{
    function find_variables(s)
    {
        var _variables = [[], [], []];

        var re1 = /(?:[^a-z0-9$_])(?:scope\[\")([a-z0-9$_]+)(?:\"\])/ig;
        var re2 = /(?:[^a-z0-9$_])(?:scope\[\')([a-z0-9$_]+)(?:\'\])/ig;
        var re3 = /(?:[^a-z0-9$_])(?:scope.)([a-z0-9$_]+)(?:[^a-z0-9$_])/ig;

        var match;
        while ((match = re1.exec(s)) !== null)
        {
            _variables[0].push(match[1]);
        }
        while ((match = re2.exec(s)) !== null)
        {
            _variables[1].push(match[1]);
        }
        while ((match = re3.exec(s)) !== null)
        {
            _variables[2].push(match[1]);
        }
    ...
    function get_find_for_replace(i, value)
    {
        switch (i)
        {
            case 0: return "scope[\"" + value + "\"]";
            case 1: return "scope[\'" + value + "\']";
            case 2: return "scope." + value;
            default:
                return "err";
        }
    }

    var _variables = find_variables(s);
    var _find, _replace;

    for (var index = 0; index < 3; index++)
    {
        for (var i = _variables[index].length - 1; i >= 0; i--)
        {
            var _find = get_find_for_replace(index, _variables[index][i]);
            var _replace = get_variable(_variables[index][i]);

            s = s.split(_find).join(_replace);
        }
    }
